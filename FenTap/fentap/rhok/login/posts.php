	...
	{  
		...
		//echo "INSERT INTO facebook_posts (post,f_name,userip,date_created) VALUES('".checkValues($_REQUEST['value'])."','$UserName','".$userip."','".strtotime(date("Y-m-d H:i:s"))."')";
		
		mysql_query("INSERT INTO facebook_posts (post,f_name,userip,date_created) VALUES('".checkValues($_REQUEST['value'])."','$username','".$userip."','".strtotime(date("Y-m-d H:i:s"))."')");
	...
	while ($row = mysql_fetch_array($result))
	{
	   ...
	   <img src="small.jpg" style="float:left;" width="33" height="33" alt="" />
		   ...
		   // $username=$_SESSION["SESS_USERNAME"];
			$userip = $_SERVER['REMOTE_ADDR'];
			if($row['f_name']==$_SESSION['SESS_USERNAME']){?>
			...
			<div id="CommentPosted<?php  echo $row['p_id']?>">
				...
				if($comment_num_row > 0)
				{
					...
					<div class="commentPanel" id="record-<?php  echo $rows['c_id'];?>" align="left">
						...
						<label  class="names">
		                <b><?php echo $rows['f_name'];?></b> 
						<!--<label class="postedsComments">-->
							<c><?php  echo clickable_link($rows['comments']);?></c></label>
							<!--</label>-->
										
					   
						
						<br clear="all" />
						<span style="margin-left:60px;  color:#666666; background-colors:#fff; font-weight:bolder; font-size:11px">
						<?php
						...
						<?php
						$userip = $_SERVER['REMOTE_ADDR'];
						if($rows['f_name']==$_SESSION['SESS_USERNAME']){?>
